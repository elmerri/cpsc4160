  ...
  channels()
{
  ...
  music = Mix_LoadMUS("sound/gameTrack.wav");
  // Need to install midi to play the following:
  // music = Mix_LoadMUS("sound/ballad2.mid");
  if (!music) throw string("Couldn't load gameTrack.wav")+Mix_GetError();

  startMusic();

  // sounds.push_back( Mix_LoadWAV("sound/gameTrack2.wav") );
...
void SDLSound::switchBackground(bool nightOn) {
  if(nightOn) {
    Mix_PauseMusic();
    Mix_FreeMusic(music);
    music = Mix_LoadMUS("sound/gameTrack2.wav");
    startMusic();
  }
  else {
    Mix_PauseMusic();
